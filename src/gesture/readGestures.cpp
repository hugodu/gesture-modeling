...
vector<GestureSample> readGestureSet(string gid)
{
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i <= 5; ++i)
	{
		vector<GestureSample> train = readGestureSet(gidPre + boost::lexical_cast<std::string>(i), "4");
	...
	for (int i = 1; i <= 5; ++i)
	{
		...
		for(int sampleNum = 0; sampleNum < transformTest.size(); sampleNum++)
		{
			...
			cout << "Gesture: " << i << " Sample: " << sampleNum << "\tClassified as: " << classifiedAs << endl;
			if(classifiedAs == i)
				correct++;
		}
	}
